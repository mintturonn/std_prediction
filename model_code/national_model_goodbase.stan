...
transformed parameters {
  real<lower = 0, upper=1> ct = ct_d * d / d_ct;   //
...
model {
  ...
  // Likelihood:
   target +=  binomial_lpmf(CT | CTN, ct) + binomial_lpmf(CTnD | CTnDN, ct_and_d) + binomial_lpmf(TnD | TnDN, t_and_d) + binomial_lpmf(T | TN, t) ; //+  + beta_lpdf(alpha_CTnD | ct_and_d, beta_CTnD); //  binomial_lpmf(K | N, theta) + binomial_lpmf(k2 | N2, theta2); //
}
