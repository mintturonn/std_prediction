...
################################
ct_data <- list(CT = 301, CTN = 10293, CTnD = 6, CTnDN = 7443, TnD =  106, TnDN = 5638, T = 1626, TN = 5641) 

fit_ct <- stan(file = "model_code/national_model.stan", data = ct_data)
print(fit_ct)
...
#################################
post <- array(0, dim=c(1000,5,14), 
              dimnames = list(NULL,
                              c("chain:1", "chain:2", "chain:3", "chain:4", "chain:X"),
                              c("d","t_ct","ct_t","d_t","ct_d","d_ct","ct","t","ct_and_d", "t_and_d", "ct_and_t", "ctN", "ct_tN", "lp_" )))
post[,1:4,] <- fit_post

post[,5,1] <- rbeta(1000, 1.5, 3) # d
post[,5,2] <- rbeta(1000, 1.5, 3) # t_ct
post[,5,3] <- rbeta(1000, 1.5, 3) # ct_t
post[,5,4] <- rbeta(1000, 1.5, 3) # d_t
post[,5,5] <- rbeta(1000, 1.5, 3) # ct_d
post[,5,6] <- rbeta(1000, 1.5, 3) # d_ct


mcmc_dens_overlay(post, pars = c("d", "t_ct", "ct_t", "d_t", "ct_d","d_ct")) +
...
plot_grid(f1, f2, f3, labels = c('A)', 'B)', "C)"), label_size = 10, nrow =3)
...
plot_grid(p1, p2, p3, p4, labels = c('A)', 'B)', "C)", "D)"), label_size = 12, nrow =2)
