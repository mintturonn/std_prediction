...
ggplot( aes(x = year)) +
  ...
  labs(x = "Year", y = "Prevalence per capita") + theme_light() + mytheme2 -> p0

ggsave(filename = here("XXXXXXXX.png"),
       plot = p0,
       device = png(),
       scale = 1, 
       width = 15,
...
ggplot( aes(x = year)) +
  ...
  facet_wrap(~age, ncol=5) + ylim(c(0,NA)) +
  labs(x = "Year", y = "Testing per capita") + theme_light() + mytheme2 
...
ggplot(clb_pos_ct, mapping = aes(x = t)) +
  ...
  labs(x = "Year", y = "positivty among tested") + mytheme2 

## men
# clb_pos <- cbind( as.data.frame(summary(fit_ct, pars = "q_ct_m", probs = c(0.025, 0.5, 0.975))$summary) , 
#                   age = fig_dat_f$age_c, t= fig_dat_m$index_year, state = fig_dat_m$state_2, data = fig_dat_m$num_test_ct/fig_dat_m$den_test_ct)
# colnames(clb_pos) <- make.names(colnames(clb_pos)) # to remove % in the col names
# 
# ggplot(clb_pos, mapping = aes(x = t)) +
#   geom_ribbon(aes(ymin = X2.5., ymax = X97.5., fill=age), alpha = 0.65) +
#   geom_line(mapping = aes(x = t, y = X50., color=age)) + 
#   geom_point(mapping = aes(y = data, color=age), shape=21) + 
...
clb_pr_ct <- cbind( as.data.frame(summary(fit_ct, pars = "p_ct_f", probs = c(0.025, 0.5, 0.975))$summary) , 
                    age = rep(c("15-24", "25-34", "35-44", "45-54", "55+"), 52),
                    state = rep(unique(fig_dat_f$state_2),each=5) )
colnames(clb_pr_ct) <- make.names(colnames(clb_pr_ct)) # to remove % in the col names

ggplot(clb_pr_ct) +
  geom_pointrange(aes(x=age, y = X50., ymin = X2.5., ymax = X97.5., color=age), alpha = 0.45, size=0.3) +
  facet_geo(~state, grid = "us_state_with_DC_PR_grid1") + # , scales="free"
  labs(x = "Age", y = "prevalence") + mytheme2 
...
## men
# clb_diag <- cbind( as.data.frame(summary(fit_ct, pars = "d_gc_m", probs = c(0.025, 0.5, 0.975))$summary) , 
#                    age = fig_dat_f$age_c, t= fig_dat_f$index_year, state = fig_dat_m$state_2, data = fig_dat_m$cases_gc/fig_dat_m$population_gc)
# colnames(clb_diag) <- make.names(colnames(clb_diag)) # to remove % in the col names
# 
# ggplot(clb_diag, mapping = aes(x = t)) +
#   geom_ribbon(aes(ymin = X2.5., ymax = X97.5., fill=age), alpha = 0.35) +
#   geom_line(mapping = aes(x = t, y = X50., color=age)) + 
#   geom_point(mapping = aes(y = data, color=age), shape=21) +
#   facet_geo(~state, grid = "us_state_with_DC_PR_grid1") + ylim(c(0, 0.02)) +
#   labs(x = "Year", y = "Diagnoses per capita") + theme_light() + mytheme2 
...
## men
# clb_pos <- cbind( as.data.frame(summary(fit_ct, pars = "q_gc_m", probs = c(0.025, 0.5, 0.975))$summary) , 
#                   age = fig_dat_f$age_c, t= fig_dat_m$index_year, state = fig_dat_m$state_2, data = fig_dat_m$num_test_gc/fig_dat_m$den_test_gc)
# colnames(clb_pos) <- make.names(colnames(clb_pos)) # to remove % in the col names
# 
# ggplot(clb_pos, mapping = aes(x = t)) +
#   geom_ribbon(aes(ymin = X2.5., ymax = X97.5., fill=age), alpha = 0.35) +
