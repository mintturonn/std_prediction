...
q <- c( 1/ ( durpars$p75[durpars$Param=="clear.f"] / 365),
        1/ ( durpars$Median[durpars$Param=="clear.f"] / 365),
        1/ ( durpars$p25[durpars$Param=="clear.f"] / 365 ))

# fit.clear.f <- rriskFitdist.perc(p, q, show.output = FALSE)

# fit.clear.f <- get.beta.par(p, q, show.output = FALSE, plot = TRUE, tol = 0.00001)

fit.clear.f <- get.lnorm.par(p, q, show.output = FALSE, plot = TRUE, tol = 0.0001)



quantile(rlnorm(100000, fit.clear.f[1], fit.clear.f[2]), probs = c(0.25, 0.5, 0.75))
q

fit.clear.f

# MEN natural clearnace duration given in days, change to monthly rate
q <- c( 1/ ( durpars$p75[durpars$Param=="clear.m"]/ 365 ),
        1/ ( durpars$Median[durpars$Param=="clear.m"]/ 365),
        1/ ( durpars$p25[durpars$Param=="clear.m"]/ 365))

# fit.clear.m <- get.beta.par(p, q, show.output = FALSE, plot = TRUE, tol = 0.001)
fit.clear.m <- get.lnorm.par(p, q, show.output = FALSE, plot = TRUE, tol = 0.0001)
quantile(rlnorm(100000, fit.clear.m[1], fit.clear.m[2]), probs = c(0.25, 0.5, 0.75))
q
fit.clear.m
...
q <- c(  ( durpars$p25[durpars$Param=="dursympt.f"]/365),
         ( durpars$Median[durpars$Param=="dursympt.f"]/365),
         ( durpars$p75[durpars$Param=="dursympt.f"]/365))

# rriskFitdist.perc(p, q, show.output = FALSE)

fit.symptclear.f <- get.beta.par(p, q, show.output = FALSE, plot = TRUE, tol = 0.0001)

quantile(rbeta(100000, fit.symptclear.f[1], fit.symptclear.f[2]), probs = c(0.25, 0.5, 0.75))
q
fit.symptclear.f

# MEN duration with symptomatic infection, until test seeking

q <- c(  ( durpars$p25[durpars$Param=="dursympt.m"]/365),
         ( durpars$Median[durpars$Param=="dursympt.m"]/365),
         ( durpars$p75[durpars$Param=="dursympt.m"]/365))

fit.symptclear.m <- get.beta.par(p, q, show.output = FALSE, plot = TRUE, tol = 0.0001)

quantile(rbeta(100000, fit.symptclear.m[1], fit.symptclear.m[2]), probs = c(0.25, 0.5, 0.75))
q
fit.symptclear.m
