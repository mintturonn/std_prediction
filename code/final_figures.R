...
ggplot() +
  ...
  labs(x = "Age", y = "CT prevalence (proportion)") +
...
clb_gc %>%
  ...
  labs(x = "Age", y = "GC prevalence (proportion)") + theme_light() + mytheme2  +ylim(c(0, 0.007)) -> p0
...
clb_te %>%
  ...
  labs(x = "Age", y = "CT testing coverage (proportion)") + theme_light() + mytheme2 +ylim(c(0, 0.6)) -> p1
...
clb_prdet_gc <- cbind( as.data.frame(summary(fit_main, pars = "pr_det_gc_f", probs = c(0.025, 0.5, 0.975))$summary) , 
                   age = rep(c("15-24", "25-34", "35-44", "45-54", ">=55"), each=length(unique(fig_dat_f$NAME))),   
                   state = rep(unique(fig_dat_f$state_2),5), state2 = fig_dat_f$NAME,  
                   data=NA,   data_ll=NA, data_ul=NA,
...
quantile(test$ratio[test$age=="15-24"])
...
quantile(test2$ratio[test$age=="15-24"])
quantile(test2$ratio2, na.rm=T)


##############

state_order_ct <- clb_pr_ct %>%
  filter(age == "15-24") %>%
  arrange(X50.) %>%
  pull(state2)

plot_dat_ct <- bind_rows(
  clb_diag_ct,
  clb_i_ct,
  clb_prdet_ct,
  clb_posit_ct,
  ...
  filter(state != "MD") %>%
  mutate(
    state2 = factor(state2, levels = state_order_ct),
    type = factor(
      type, levels = c("diagnoses", "positivity","incidence", "prevalence", "pr detected"),
      labels = c( "Diagnosis rate", "Positivity", "Incidence", "Prevalence", "Probability detected")))
...
plot_dat_ct %>%
  ...
  ggplot(aes(y = state2)) +
  geom_vline(
    xintercept = 0, color = "black", linewidth = 0.2) +
  geom_pointrange(
    aes( x = X50., xmin = X2.5., xmax = X97.5., color = type), linewidth = 0.25 ) +
  geom_pointrange(
    aes( x = data, xmin = data_ll, xmax = data_ul), color = "black", shape = 1, alpha = 0.4) +
  facet_grid(rows = vars(age), cols = vars(type), scales = "free_x", space = "fixed",  switch = "y") + 
  labs( x = "Estimate", y = NULL, color = NULL ) +
  scale_x_continuous(labels = scales::label_percent(accuracy = 0.1)) +
  mytheme4 +
  theme(legend.position = "none",  
        strip.text.x = element_text(face = "bold", size = 10),
        strip.text.y.left = element_text(angle = 0, face = "bold", hjust = 0, vjust = 1, size = 10 ),
        strip.placement = "outside",
        strip.background = element_blank(),
        panel.spacing.x = unit(0.5, "lines"),
        panel.spacing.y = unit(0.7, "lines")) -> p1
...
plot_dat_ct %>%
  ...
  theme(legend.position = "none",  
    ...
    strip.background = element_blank(),
    panel.spacing.x = unit(0.5, "lines"),
    panel.spacing.y = unit(0.7, "lines")) -> p2
...
state_order_gc <- clb_pr_gc %>%
  filter(age == "15-24") %>%
  arrange(X50.) %>%
  pull(state2)

plot_dat_gc <- bind_rows(
  clb_diag_gc,
  clb_i_gc,
  clb_prdet_gc,
  clb_posit_gc,
  ...
  filter(state != "MD") %>%
  mutate(
    state2 = factor(state2, levels = state_order_gc),
    type = factor(
      type, levels = c("diagnoses", "positivity","incidence", "prevalence", "pr detected"),
      labels = c( "Diagnosis rate", "Positivity", "Incidence", "Prevalence", "Probability detected")))
...
plot_dat_gc %>%
  ...
  theme(legend.position = "none",  
        ...
        strip.background = element_blank(),
        panel.spacing.x = unit(0.5, "lines"),
        panel.spacing.y = unit(0.7, "lines")) -> p3
...
plot_dat_gc %>%
  ...
  theme(legend.position = "none",  
        ...
        strip.background = element_blank(),
        panel.spacing.x = unit(0.5, "lines"),
        panel.spacing.y = unit(0.7, "lines")) -> p4
...
post_df %>%
  ...
  filter(parameters!= "Prob.symptoms,CT" & parameters!= "Prob.symptoms,GC") %>%
  ggplot(aes(x = parameters, y = 1/X50., color = estimate)) +
  geom_pointrange(aes(ymin = 1/X97.5., ymax = 1/X2.5. ), 
                  position = position_dodge(width = 0.7),
                  size = 0.6) +
  scale_color_manual(values = c("Prior" = "steelblue4", "Posterior" = "maroon")) +
  labs(x = "",y = "Duration (years)",color = "") + ylim(c(0,NA)) +
...
ggsave(filename = here("figs/calib-params2-1.png"),
       plot = p3,
       device = png(),
       scale = 1, 
       width = 15,
...
post_df %>%
  ...
  geom_pointrange(aes(ymin = X2.5., ymax = X97.5. ), 
                  ...
                  size = 0.6) +
  scale_color_manual(values = c("Prior" = "steelblue4", "Posterior" = "maroon")) +
  labs(x = "",y = "Proportion symptomatic",color = "") + ylim(c(0,NA)) +
  theme_bw(base_size = 10) + 
  theme(axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(),
        legend.position = "bottom") +
...
ggsave(filename = here("figs/calib-params2-2.png"),
       plot = p3,
       device = png(),
       scale = 1, 
       width = 15,
