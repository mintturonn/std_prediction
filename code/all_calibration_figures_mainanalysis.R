...
ggplot() +
  ...
  geom_pointrange(data =ct_data2, aes(x = age, y = CTp, ymin=CTp-1.96*CTse,  ymax=CTp+1.96*CTse), color="maroon", 
                  shape=21, alpha=0.2) +
  labs(x = "Age", y = "CT prevalence per capita") + theme_light() + mytheme2  +ylim(c(0, 0.1)) -> p0
...
clb_gc %>%
  ...
  ggplot() +
  geom_pointrange( aes(x = age, y = X50., ymin=  X2.5.,  ymax=X97.5.), color="orange") +
  labs(x = "Age", y = "GC prevalence per capita") + theme_light() + mytheme2  +ylim(c(0, 0.1)) -> p0
...
clb_te %>%
  ...
  geom_pointrange(data =ct_data1, aes(x = age, y = Tp, ymin=Tp-1.96*Tse,  ymax=Tp+1.96*Tse), color="maroon", 
                  shape=21, alpha=0.2) +
  labs(x = "Age", y = "CT testing coverage per capita") + theme_light() + mytheme2 +ylim(c(0, NA)) -> p1
...
rbind(clb_diag_ct, clb_posit_ct, clb_pr_ct, clb_i_ct, clb_prdet_ct) %>%
...
rbind(clb_diag_ct, clb_i_ct, clb_prdet_ct, clb_posit_ct, clb_pr_ct) %>% # 
  # filter(type=="diagnoses" | type=="incidence" | type=="pr detected") %>%
  filter(age !="15-24" & age !="25-34") %>%
  filter(state!="HI" & state!="PR" & state!="ND" & state!="SD" & state!="ME" & state!="VT" & state!="NH" & state!="CT"  & state!="MD" ) %>%
  ggplot() +
  geom_vline(xintercept = 0, color = "black") + 
  geom_pointrange(aes(y=state2, x = X50., xmin = X2.5., xmax = X97.5., color=type), 
                  size = 0.4) +
  geom_pointrange(aes(y=state2, x = data, xmin=data_ll, xmax=data_ul),
                  color="black", shape=1,  alpha=0.4) +
  facet_grid(~age+type, scales="free") + xlab("Estimate per capita") + mytheme4  + ylab("")  -> p2
...
rbind(clb_diag_gc, clb_posit_gc, clb_pr_gc, clb_i_gc, clb_prdet_gc) %>%
  filter(age =="15-24" | age =="25-34") %>%
  filter(state!="HI" & state!="PR" & state!="ND" & state!="SD" & state!="ME" & state!="VT" & state!="NH" & state!="CT" & state!="MD" ) %>%
  # filter(state!="ME" & state!="PR" & state!="VT" & state!="NH" & state!="RI" & state!="DE" & state!="CT" &
  #          state!="NH" & state!="HI" & state!="NE" & state!="ID"  & state!="ND" & state!="SD" & state!="WY") %>%
...
rbind(clb_pr_ct) %>%
 ...
 # filter(age =="15-24" | age =="25-34") %>%
  ...
  facet_grid(~age, scfit_mainales="free") + xlab("Ratio of prevalence to diagnosis rate") + mytheme4  + ylab("") + xlim(c(0,NA))
...
clb_testcov <- cbind( as.data.frame(summary(fit_main, pars = "test_cov", probs = c(0.025, 0.5, 0.975))$summary) , 
                    age = rep(c("15-24", "25-34", "35-44", "45-54", "55+"), each=length(unique(fig_dat_f$NAME))),
                    state = rep(unique(fig_dat_f$state_2),5), state2 = fig_dat_f$NAME)
...
## men
# clb_diag <- cbind( as.data.frame(summary(fit_main, pars = "d_ct_m", probs = c(0.025, 0.5, 0.975))$summary) , 
...
clb_pos_ct <- cbind( as.data.frame(summary(fit_main, pars = "q_ct_f", probs = c(0.025, 0.5, 0.975))$summary) , 
...
####### GC
## women
clb_diag <- cbind( as.data.frame(summary(fit_main, pars = "d_gc_f", probs = c(0.025, 0.5, 0.975))$summary) , 
...
##
## women
clb_pos_gc <- cbind( as.data.frame(summary(fit_main, pars = "q_gc_f", probs = c(0.025, 0.5, 0.975))$summary) , 
...
##############################
# estimation, testing
clb_tr_f <- cbind( as.data.frame(summary(fit_main, pars = "tr_f", probs = c(0.025, 0.5, 0.975))$summary) , 
...
clb_tr_m <- cbind( as.data.frame(summary(fit_main, pars = "tr_m", probs = c(0.025, 0.5, 0.975))$summary) , 
