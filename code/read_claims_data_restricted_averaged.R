...
read.csv(here("data/cdc_gc_ct_state.csv"), skip=7) %>%
  ...
  group_by(NAME, Gender, age_c, infection) %>%
  ...
  select(NAME, Gender, age_c, cases_ct, cases_gc, population_ct, population_gc, rate_ct, rate_gc) -> cdc_diag_ct_gc

#  
read.csv("/Users/minttu/restricted_files/ct_state_sex_age.csv") %>%
  mutate(positivity_ct = num_test/den_test) %>% 
  rename(num_test_ct= num_test,
         den_test_ct= den_test) -> ct_age_sex


read.csv("/Users/minttu/restricted_files/gc_state_sex_age.csv") %>%
...
cdc_diag_ct_gc %>% 
  ...
  left_join(ct_age_sex, by= c("state_2", "Gender", "age_c")) %>%
  left_join(gc_age_sex, by= c("state_2", "Gender", "age_c")) %>% 
  mutate(pop_prop = den_test_ct/population_ct) %>%
  mutate(age_c = factor(age_c, levels= c("15-24", "25-34", "35-44", "45-54", ">=55"))) -> gc_ct_age_sex
...
ctnum_f_array <- array(dat_f$num_test_ct, dim = c(age_num, state_num))
ctden_f_array <- array(dat_f$den_test_ct, dim = c(age_num, state_num))
...
# gonorrhea positivity data
# gcnum_f_array <- array(dat_f$num_test_gc2, dim = c(years_num, age_num, state_num))
gcnum_f_array <- array(dat_f$num_test_gc, dim = c(age_num, state_num))
gcden_f_array <- array(dat_f$den_test_gc, dim = c(age_num, state_num))
...
ctdnum_f_array <- array(dat_f$cases_ct, dim = c(age_num, state_num))
ctdden_f_array <- array(dat_f$population_ct, dim = c(age_num, state_num))
...
gcdnum_f_array <- array(dat_f$cases_gc, dim = c(age_num, state_num))
gcdden_f_array <- array(dat_f$population_gc, dim = c(age_num, state_num))
...
# population
pop_f_array <- array(dat_f$population_ct, dim = c(age_num, state_num))
...
gc_ct_age_sex %>%
  ...
  mutate(num_test_ct2 = ifelse(is.na(num_test_ct) & !is.na(den_test_ct), 1, num_test_ct)) %>%
  mutate(num_test_gc2 = ifelse(is.na(num_test_gc) & !is.na(den_test_gc), 1, num_test_gc)) %>%
  arrange(state_2, age_c) -> dat_m

# chlamydia positivity data
# ctnum_m_array <- array(dat_m$num_test_ct2, dim = c(years_num, age_num, state_num))
ctnum_m_array <- array(dat_m$num_test_ct, dim = c(age_num, state_num))
ctden_m_array <- array(dat_m$den_test_ct, dim = c(age_num, state_num))
...
gcnum_m_array <- array(dat_m$num_test_gc, dim = c(age_num, state_num))
gcden_m_array <- array(dat_m$den_test_gc, dim = c(age_num, state_num))
...
# chlamydia diagnosis data
ctdnum_m_array <- array(dat_m$cases_ct, dim = c(age_num, state_num))
ctdden_m_array <- array(dat_m$population_ct, dim = c(age_num, state_num))
# ! HERE denominator has 
...
# gonorrhea diagnosis data
gcdnum_m_array <- array(dat_m$cases_gc, dim = c( age_num, state_num))
gcdden_m_array <- array(dat_m$population_gc, dim = c( age_num, state_num))
...
# population
pop_m_array <- array(dat_m$population_ct, dim = c( age_num, state_num))
